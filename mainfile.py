...
def display_plot():
        ...
        for i in range(10,110,10):
            ...
            """
            plt.bar(5,98,animated=1)
            plt.bar(6,88,animated=1)
...
def box():
    ...
    canvas.create_window(670,300,window=username,height=35,width=200)
    canvas.create_text(475,300,fill="black",font="Bradley 25 ",
                        text="Username ")
    
    
    canvas.create_text(475,400,fill="black",font="Bradley 25 ",
                        text="Password ")
    password= Entry(canvas)
    canvas.create_window(670,400,window=password,height=35,width=200)
    
    user = username.get()
    print(str(user))
    
    button1 = Button(root, text = "Login", command = afterlogin, anchor = W)
    button1.configure(width = 10,height=2, activebackground = "#33B5E5", relief = RAISED,font="Bradley 20 ",anchor=CENTER)
    #button1.place(x=)
    button1_window = canvas.create_window(500, 500, anchor=NW, window=button1)
    """
